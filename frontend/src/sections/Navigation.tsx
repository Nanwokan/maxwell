...
const Navigation = ({
  settings = defaultPublicHomepageData.settings,
}: NavigationProps) => {
  const [isScrolled, setIsScrolled] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const navRef = useRef<HTMLElement>(null);
  const brandLabel = settings.clubName
    .replace(/^Association Sportive\s+/i, "")
    .toUpperCase();
  ...
  const getScrollTargetFromTrigger = useCallback(
    (element: HTMLElement, href: string, offset: number) => {
      ...
      const pinnedTrigger = matchingTriggers.find((trigger) =>
      ...
      if (hasPin && hasFiniteEnd) {
        ...
        const segmentLength = Math.max(
          0,
          chosenTrigger.end - chosenTrigger.start,
        );
        return Math.max(
          0,
          chosenTrigger.start + segmentLength * sectionProgress - offset,
        );
      }
    ...
    (href: string) => {
      ...
      if (element) {
        ...
        if (ScrollTrigger.getAll().length > 0) {
          ...
          targetTop =
            getScrollTargetFromTrigger(element, href, offset) ?? targetTop;
        }
